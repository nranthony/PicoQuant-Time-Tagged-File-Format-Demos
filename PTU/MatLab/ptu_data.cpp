...
public:
    ...
    void operator()(ArgumentList outputs, ArgumentList inputs) {
        ...
        ProcessTTTRData(macrotVect, microtVect);
        ...
        if (outputs.size() == 4) {
            ...
            CellArray metaCellArr = af.createCellArray({ 2,10 }
                ...
                , af.createScalar<uint32_t>(AcqTime), af.createScalar("AcqTime")
                ...
                , af.createScalar<double>(TimePerPix), af.createScalar("TimePerPix")
                //TTTRTagPixRes       double PixRes = 0.0;          // Physicsal pixel resolution
                , af.createScalar<double>(PixRes), af.createScalar("PixRes")
                //TTTRTagSyncRate     unsigned long SyncRate = 0;   // Laser pulse sync rate frequency in Hz
                , af.createScalar<uint32_t>(SyncRate), af.createScalar("SyncRate")
                ...
                , af.createScalar<double>(Resolution), af.createScalar("Resolution")
                //TTTRTagGlobRes      double GlobRes = 0.0;         // Global Resolution of TimeTag(T2)/NSync (T3)
                , af.createScalar<double>(GlobRes), af.createScalar("GlobRes")     
            );                                        
            outputs[3] = metaCellArr;
        }
    ...
    void ProcessTTTRData(std::vector<uint64_t> &macrotVect, std::vector<uint16_t> &microtVect)
    {
