...
public:
    ...
    void operator()(ArgumentList outputs, ArgumentList inputs) {
        ...
        TypedArray<uint64_t> Amac = factory.createArrayFromBuffer<uint64_t>({ macrotVect.size(), 1 }, std::move(Vmac_b_ptr));
        outputs[0] = Amac;

        // these objects are std::unique_ptr<T[],matlab::data::buffer_deleter_t>
        auto Vmic_b_ptr = factory.createBuffer<uint16_t>(microtVect.size());
        // this pointer points to the first adress that the std::unique_ptr points to
        uint16_t* Vmic_ptr = Vmic_b_ptr.get();
        // fill the buffer with the V values
        std::for_each(microtVect.begin(), microtVect.end(), [&](const uint16_t& e) { *(Vmic_ptr++) = e; });
        // finally create Matlab array from buffer
        TypedArray<uint16_t> Amic = factory.createArrayFromBuffer<uint16_t>({ microtVect.size(), 1 }, std::move(Vmic_b_ptr));
        outputs[1] = Amic;

        // histogram microtime data
        auto dec_b_ptr = factory.createBuffer<uint32_t>(1024);
        ...
        TypedArray<uint32_t> Adec = factory.createArrayFromBuffer<uint32_t>({ 1024, 1 }, std::move(dec_b_ptr));
        outputs[2] = Adec;

    }

    void readHeader(ArgumentList inputs) {
        ...
        // TODO - Use _splitpsth_s to pull filename for output
        // see https://www.tenouk.com/cpluscodesnippet/makepath_splitpath_s.html for example
    ...
    void ProcessTTTRData()
    {
        ...
        for (RecNum = 0; RecNum < NumRecords; RecNum++)
        {
            ...
            if (T3Rec.bits.special == 1)
            {
                ...
                /*else if ((T3Rec.bits.channel >= 1) && (T3Rec.bits.channel <= 15)) //markers - ignoring for now
                {
    ...
    void displayOnMATLAB(std::ostringstream& stream) {
        ...
        matlabPtr->feval("fprintf", 0, std::vector<Array>({ factory.createScalar(stream.str()) }));
        // Clear stream buffer
        stream.str("");
    }
